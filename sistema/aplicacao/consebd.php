   ...
   $FORM_NAME = 'Classe Escola Bíblica Dominical';
	...
	class MyPag extends Paginacao
	{
		 	 ...
		 	 $html = '
			        <thead>
					 <tr valign="top" class="tab_bg_2"> 
                      ...
                      <th width="8%">Qtd Aluno</th>
                      <th width="20%">Lição Bíblica</th>
		...
		function desenha($row){
			...
			$sql2 =" select count(*) as total from aluno_classe_licao_trimestre aclt
inner join classe_licao_trimestre clt on aclt.idclasse_licao_trimestre = clt.idclasse_licao_trimestre
where aclt.idclasse_licao_trimestre = ".$row['idclasse_licao_trimestre'];

$row2 = pg_fetch_array($rs2 = pg_query($sql2));
$qtdaluno = $row2['0'];
					
			//converte datas
			$data_nas = date('d/m/Y',strtotime($row['datanas']));					
			$html = ' 
                      <td align="center"><input type="checkbox" name="id_[]" id="id_" value="'.$row['idclasse_licao_trimestre'].'" /></td>
					  <td nowrap><a href="consaula.php?op=A&id='.$row['idclasse_licao_trimestre'].'">'.$row['classe'].'</a></td>
					  <td nowrap>'.$qtdaluno.'</td>
					  <td nowrap>'.$row['licao'].'</td>
					  <td nowrap>'.$row['trimestre'].'</td>
	...
	$sql = "SELECT * FROM classe c
			INNER JOIN classe_licao_trimestre clt on c.idclasse = clt.idclasse
	...
	}if ($cmboxordenacao=='trimestre')
	{
		...
		$sql.= " order by ano desc,trimestre desc,2 ";
	}
	//print $sql;
