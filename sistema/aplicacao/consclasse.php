...
header('Location: index.php?erro=1');
   ...
   $TABLE_NAME = 'classe';
	...
	if ($retorno == 'INS_OK')
	{
		$MENSAGEM_RETORNO = $FORM_NAME.' adicionada com sucesso!';
		$MENSAGEM_COLOR = 'success';
	}
	if ($retorno == 'ALT_OK')
	{
		$MENSAGEM_RETORNO = $FORM_NAME.' alterada com sucesso!';
		$MENSAGEM_COLOR = 'success';
	}
	if ($retorno == 'DEL_OK')
	{
		$MENSAGEM_RETORNO = $FORM_NAME.' exclu&iacute;da com sucesso!';
		$MENSAGEM_COLOR = 'success';
	}
	if ($retorno == 'INS_ERRO')
	{
		$MENSAGEM_RETORNO = 'NÃ£o foi possÃ­vel adicionar a '.$FORM_NAME.'!';
		$MENSAGEM_COLOR = 'warning';
	}
	if ($retorno == 'ALT_ERRO')
	{
		$MENSAGEM_RETORNO = 'NÃ£o foi possÃ­vel alterar a '.$FORM_NAME.'!';
		$MENSAGEM_COLOR = 'warning';
	}
	if ($retorno == 'DEL_ERRO')
	{
		$MENSAGEM_RETORNO = 'NÃ£o foi possÃ­vel excluir a '.$FORM_NAME.'!';
		$MENSAGEM_COLOR = 'warning';
	}
	...
	class MyPag extends Paginacao
	{
		 	 ...
		 	 $html = '
			        <thead>
					 <tr valign="top" class="tab_bg_2"> 
                      <th width="3%">#</th>
                      <th width="37%">Classe</th>
                      <th width="20%">Sigla Classe</th>
		...
		function desenha($row){
			...
			$html = ' 
                      <td align="center"><input type="checkbox" name="id_[]" id="id_" value="'.$row['idclasse'].'" /></td>
					  <td nowrap><a href="cadclasse.php?op=A&id='.$row['idclasse'].'">'.$row['classe'].'</a></td>
					  <td nowrap><a href="cadclasse.php?op=A&id='.$row['idclasse'].'">'.$row['siglaclasse'].'</a></td>					  
					  ';
		 		echo $html;
				echo "";
		}// function
	...
	/*$sql = "SELECT c.*,l.licao,clt.idclasse_licao_trimestre,clt.idlicao ,clt.idtrimestre , t.trimestre,t.ano 
FROM classe c, classe_licao_trimestre clt
INNER JOIN licao l on l.idlicao = clt.idlicao 
INNER JOIN trimestre t on t.idtrimestre = clt.idtrimestre 
where c.idclasse = clt.idclasse   ";	
	*/
    ...
    <div id="page-wrapper">
       ...
       <div id="page-wrapper">
        <div class="row">
		  ...
		  <div class="col-lg-12"> 
			<h1 class="page-header">Classes da Escola Bilica Dominical</h1>
			<?php if ($retorno!='')	{ ?>
