...
header('Location: index.php?erro=1');
   ...
   $TABLE_NAME = 'trimestre';
	...
	class MyPag extends Paginacao
	{
		...
		function desenha($row){
			...
			$html = ' 
					  ...
					  <td nowrap><a href="cadtrimestre.php?op=A&id='.$row['idtrimestre'].'">'.$row['trimestre'].'</a></td>
					  <td nowrap>'.$row['ano'].'</td>
					  <td nowrap>'.$row['datainicio'].'</td>
					  <td nowrap>'.$row['datafim'].'</td>
	...
	$trimestre = new EscolaBiblica(); // <-- Alterar o nome da trimestre
	$trimestre->conn = $conn;
	
	$filtrotrimestre = $_REQUEST['edtfiltrotrimestre'];
	$cmboxordenacao = $_REQUEST['cmboxordenacao'];
	
	$filtroigreja = $_REQUEST['cmboxigreja'];

	$sql = "SELECT * FROM trimestre where idtrimestre = idtrimestre ";	
	
	$filtroopen = false;
	if (!empty($filtrotrimestre))
	{
		$sql.=" and trimestre ilike '%".$filtrotrimestre."%'";
		$filtroopen = true;
	}
	...
	}if ($cmboxordenacao=='trimestre')
	{
		...
		$sql.= " order by ano desc, trimestre ";
	}
	//print $sql;
    ...
    <div id="page-wrapper">
       ...
       <div id="page-wrapper">
        <div class="row">
		  ...
		  <div class="col-lg-12"> 
			<h1 class="page-header">Trimestres da Escola Bilica Dominical</h1>
			<?php if ($retorno!='')	{ ?>
            ...
            <div id="collapseOne" class="panel-collapse collapse <?php if ($filtroopen==true){ echo "in";} else {echo "out";}?>"> 
              <div class="panel-body"> 
                <div class="row-fluid"> 
                  <div class="row"> 
                    <div class="col-lg-6"> 
                      <div class="input-group input-group-sm"> <span class="input-group-addon">trimestre</span> 
                        <input type="text" class="form-control"  name="edtfiltrotrimestre" id="edtfiltrotrimestre" value="<?php echo $filtrotrimestre;?>">
                      </div>
                    </div>
                    <div class="col-lg-3"> 
                      <div class="input-group input-group-sm"> <span class="input-group-addon">Ordenado 
                        ...
                        <select name="cmboxordenacao" id="cmboxordenacao"  class="form-control" placeholder="Order">
                          ...
                          <option value="trimestre" <?php if ($cmboxordenacao=='trimestre') echo "selected";?> >trimestre</option>
                           </select>
                      </div>
                    </div>
